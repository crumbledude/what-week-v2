...
def create_media_container(image_url):
    ...
    if response.status_code != 200:
        raise requests.exceptions.HTTPError(
            f"Failed to create media container: {response.json()}"
        )
...
def publish_media_container(media_id):
    response = requests.post(
        f"{ENDPOINT}/threads_publish",
        params={"creation_id": media_id, "access_token": THREADS_ACCESS_TOKEN},
        timeout=30,
    )

    if response.status_code != 200:
        raise requests.exceptions.HTTPError(
            f"Failed to publish media container: {response.json()}"
        )
...
if __name__ == "__main__":
    media_id = create_media_container(IMG_URL)
    ...
    publish_media_container(media_id)
