...
def lovely_art(img_bytes, week_number, author_name):
    ...
    font = ImageFont.truetype(
        "./fonts/Roboto-Bold.ttf", 290
    )  # setup font for main text
    draw.text(
        (724, 724), "Week " + str(week_number), anchor="mm", font=font
    )  # add current week text centered

    font = ImageFont.truetype("./fonts/Roboto-Bold.ttf", 95)  # setup font info text
    draw.text(
        (724, 280), "On Monday it will be at THS:", anchor="mm", font=font
    )  # add info text centered

    font = ImageFont.truetype("./fonts/Roboto-Bold.ttf", 95)  # as of date text
    current_date = datetime.now().strftime("%d-%m-%y")
    draw.text(
        (724, 1168), "As of " + str(current_date), anchor="mm", font=font
    )  # add date text centered

    font = ImageFont.truetype("./fonts/Roboto-Bold.ttf", 30)  # author text
    draw.text(
        (724, 1400), "Image by " + author_name + " on Unsplash", anchor="mm", font=font
    )  # add author text centered

    img.save("./web/img.png")
